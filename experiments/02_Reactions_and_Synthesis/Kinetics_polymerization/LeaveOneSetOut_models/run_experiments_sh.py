...
TARGET_PROPERTY_MAP = {
    ...
    'rate_bin': 'Rate of Polymerisation'
}


MAX_TEST_DATA = 50

def train_test(train_size: int = 300, random_state: int = 42, model = 'EleutherAI/gpt-j-6b', num_epochs = 30, target = 'HER_bool', representation = 'mofid', file = 0):
    try:
    ...
    train_data = [i for i in os.listdir('data') if i.startswith(f'{file}_train')][0]
    test_data = [i for i in os.listdir('data') if i.startswith(f'{file}_test')][0]
    ...
    config = {
        "property_name": "rate of polymerisation",
    ...
    """
    df_train = pd.read_csv(os.path.join('data', train_data))
    df_test = pd.read_csv(os.path.join('data', test_data))
    ...
    df_all.to_csv(os.path.join(pred_folder, f'{timestamp}_predictions_seed{random_state}_{target}_{representation}_{num_epochs}epoch_{file}.csv'))
