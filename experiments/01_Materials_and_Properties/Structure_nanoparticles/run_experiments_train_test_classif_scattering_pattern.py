...
def train_test(train_size: int = 300, random_state: int = 42):
    ...
    df = pd.read_csv("ScatteringPattern_dataset.csv", sep=',')
    target = "structure_type_cat"
    representation = "scattering_pattern"
    num_epochs = 30

    config = {
        "property_name": "number of atoms",
        "tune_settings": {"num_train_epochs": num_epochs, "learning_rate": 3e-4},
        "tokenizer_kwargs": {"cutoff_len": 1024},
    ...
    wandb.init(
        ...
        project="gpt-challenge-classif-scatteringPattern",
        ...
        tags=["classification", "scatteringPattern", "gpt-j-6b"],
    )
