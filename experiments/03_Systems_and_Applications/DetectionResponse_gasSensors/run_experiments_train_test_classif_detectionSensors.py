...
def train_test(train_size: int = 300, random_state: int = 42):
    ...
    df = pd.read_csv("DetectionResponseSensors_dataset.csv", sep=';')
    target = "sensing_2bins_5050"
    representation = "sensor_conds"
    num_epochs = 100

    config = {
        "property_name": "sensor response",
    ...
    wandb.init(
        ...
        project="gpt-challenge-classif-detectionSensors",
        ...
        tags=["classification", "detectionSensors", "gpt-j-6b"],
    )
