...
def train_test(train_size: int = 300, random_state: int = 42, model = 'EleutherAI/gpt-j-6b', num_epochs = 30, representation = 'SMILES', target = 'E_bin', data_file = 'train_polymers.csv', max_test_data = 50):
    ...
    config = {
        "property_name": "Polymer adhesion Energy",
    ...
    wandb.init(
        ...
        tags=["classification", data_file, model, data_file, representation, target],
    )
    ...
    save_pickle(
        os.path.join(OUTFOLDER, f"{timestamp}_{train_size}_{num_epochs}_predictions.pkl"),
